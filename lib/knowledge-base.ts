 ...
 * response.
 *
 * The load-bearing detail for this app's story is `requiredScope`. Retrieval
 * FILTERS by the caller's granted scopes BEFORE it ranks, so an under-scoped user
 * doesn't just fail to open a restricted article — the article never surfaces at
 * all. They can't even learn it exists.
 */
